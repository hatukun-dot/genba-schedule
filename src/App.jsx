...
function AppInner() {
  ...
  async function exportBillingXlsx(selectedIds, year, monthIndex0) {
    ...
    const selectedTargets = billingTargets.filter((t) => selectedIds.includes(t.id) && t.groupId === null);
    if (selectedTargets.length === 0) return;

    // 締め期間をカバーする範囲でイベントを取得
    const has20 = selectedTargets.some((t) => t.closingType === "20日締め");
    const period_month = monthIndex0 + 1;
    let fetchStart;
    if (has20) {
    ...
    const fetchEnd = new Date(year, monthIndex0 + 1, 1);

    const { data, error } = await api.fetchEventsForExport({
      startYmd: toYmd(fetchStart),
      endYmdExclusive: toYmd(fetchEnd),
    });
    if (error) {
    ...
    const body = {
      ...
      managers: (managersAll || []).map((m) => ({ id: m.id, name: m.name })),
      allBillingTargets: billingTargets.map((t) => ({
        id: t.id, name: t.name, projectId: t.projectId, groupId: t.groupId,
        closingType: t.closingType, outputType: t.outputType,
        billingType: t.billingType, unitPrice: t.unitPrice,
      })),
    };
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        pushError("請求書の生成に失敗しました", err.error || resp.statusText);
        return;
      }
      ...
      a.href = url;
      a.download = `${year}年${period_month}月_請求書.xlsx`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (e) {
      pushError("請求書の生成に失敗しました", e.message || String(e));
    }
  }
  // 複数日モーダル内：日付クリック
